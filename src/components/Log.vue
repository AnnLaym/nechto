    ...
    <div ref="bodyLog" class="bodyLog" @scroll="handleScroll()">
        ...
        <div v-for="message in state.gameLog" class="message">
            <span v-if="message.action && !message.panika" class="messageCard">
                <span
                    v-if="message.actors?.[0]"
                    :style="{
                        color: colors[state.playerSlots.indexOf(message.actors?.[0])],
                    }"
                    class="actor">
                ...
                <span v-if="message.nechto && message.action === 'start-game'">{{ getActionLog('s-nechto') }}</span>
                <span
                    v-if="message.actors?.[1]"
                    :style="{
                        color: colors[state.playerSlots.indexOf(message.actors?.[1])],
                    }"
                    class="actor">
                    {{ state.playerNames[message.actors[1]] }}
                </span>
            </span>
...
<!--TODO: зависает при масштабе 80%-->
<script lang="ts" setup>
    import { computed, nextTick, ref, watch } from 'vue'
    import { getActionLog, getCardLog, getKnopkaName } from '../log'
    import { useNechtoState } from '../service'
