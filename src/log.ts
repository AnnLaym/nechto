import { CardId, userLang } from './service'
...
const cardsData: PartialRecord<CardId, CardMetaData> = {
    ...
    soblazn: {
        ...
        desciption: {
            ru: 'Поменяйтесь 1 картой с любым игроком по вашему выбору, если он не на карантине. Ваш ход заканчивается',
            ua: 'Поміняйтеся 1 картою з будь-яким гравцем на ваш вибір, якщо він не на карантині. Ваш хід закінчується',
            en: 'Trade 1 card with any player of your choice, unless they are in quarantine. Your turn ends',
        },
    },
    strah: {
        log: {
    ...
    mimo: {
        ...
        desciption: {
            ru: "Откажитесь от обмена картами. Вместо вас картами меняется следующий за вами игрок. Возьмите 1 карту события. (Нечто не может играть 'Мимо!' при обмене с зараженным)",
            ua: "Відмовтеся від обміну картами. Замість вас картами обмінюється наступний за вами гравець. Візьміть 1 карту події. (Щось не може грати 'Не влучив!' при обміні з зараженим)",
            en: "Stop exchanging cards. Instead of you, the next player behind you changes cards. Draw 1 card. (Thing cannot play 'Missed!' when exchanging with an infected person)",
        },
    },
    nikakogoShahlika: {
        log: {
    ...
    razDva: {
        ...
        desciption: {
            ru: 'Поменяйтесь местами с третьим игроком слева или справа (по вашему выбору). Если игрок на карантине - смена мест не происходит',
    ...
    vremyaPriznaniy: {
        ...
        desciption: {
            ru: "Начиная с вас и по порядку, каждый игрок либо показывает, либо не показывает все карты на руке остальным игрокам. Время признаний заканчивается, когда кто-то из игроков показывает карту 'Заражение', при этом нет необходимости показывать остальные карты на руке",
            ua: "Починаючи з вас і по порядку, кожен гравець або показує, або не показує всі карти на руці іншим гравцям. Час зізнань закінчується, коли хтось із гравців показує карту 'Зараження', при цьому немає необхідності показувати інші карти на руці",
            en: "Starting with you and in order, each player either shows or does not show all the cards in his hand to the rest of the players. The time for confessions ends when one of the players shows the 'Infection' card, and there is no need to show the rest of the cards in his hand",
        },
    },
    tsepnayaReaksia: {
        log: {
...
const actionsData: Record<string, ActionMetaData> = {
    ...
    'ne-obmen': {
        log: {
            ru: 'не обменивается с',
            ua: 'не обмінюється з',
            en: 'does not exchange with',
        },
    },
    'pokaz': {
        log: {
    ...
    'test-mode': {
        log: {
            ru: 'Тест-мод включен',
            ua: 'Тест-мод включений',
            en: 'Test-mod is enabled',
        },
    },
    'pizdets': {
        log: {
